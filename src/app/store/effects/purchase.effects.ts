      ...
      this.restService.uploadNewPurchase(action.purchase)
        ...
        .subscribe((result) => {
          if (result.code !== 200) {
            ...
            observer.next(PurchaseActions.purchaseUploadSuccessful({purchase: result.purchase}));
          }
        })
    }))
  ));
      ...
      this.restService.updatePurchase(action.purchase)
        ...
        .subscribe((result) => {
          if (result.code !== 200) {
            ...
            observer.next(PurchaseActions.purchaseUpdateSuccessful({purchase: result.purchase}));
          }
        })
    }))
  ));
    ...
    mergeMap((action) => this.restService.deletePurchase(action.purchase.purchaseId)
      .pipe(
        map(result => {
          if (result.code !== 200) {
            console.log(result.code, result.message);  // TODO: Show this in a toast message
            return PurchaseActions.purchaseDeleteFailed({purchase: action.purchase});
          } else {
            return PurchaseActions.purchaseDeleteSuccessful({purchaseId: action.purchase.purchaseId});
          }
        })
      )
    )
